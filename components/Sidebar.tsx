...
export const Sidebar: React.FC<SidebarProps> = ({ currentView, onChangeView, accentColor, user, onLogout, appMode = 'kids' }) => {
  ...
  const NavItem = ({ view, icon: Icon, label }: { view: ViewState, icon: any, label: string }) => (
    <button
      ...
      className={`w-full flex items-center gap-4 px-4 py-3 transition-all duration-200 group ${appMode === 'adult' ? 'rounded-lg' : 'rounded-2xl'
        } ${currentView === view
          ? appMode === 'adult'
    ...
    <div className={`hidden md:flex flex-col w-80 xl:w-96 h-screen bg-white border-r border-slate-100 p-6 ${appMode === 'adult' ? 'w-64 xl:w-80' : ''}`}>
      ...
      <div className="flex-1 space-y-2">
        ...
        <button
          ...
          className={`w-full flex items-center gap-4 px-4 py-3 transition-all mt-6 ${appMode === 'adult'
